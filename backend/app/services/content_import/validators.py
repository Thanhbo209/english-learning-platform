...
def validate_document(data: DocumentData) -> list[ValidationIssue]:
    if not data.body or not data.body.strip():
        return [ValidationIssue(location="document", message="Document body is empty.")]
    return []


def validate_vocabulary(items: list[VocabularyItemData]) -> list[ValidationIssue]:
    if not items:
        return [ValidationIssue(location="vocabulary", message="No vocabulary items found.")]
    ...
    for index, item in enumerate(items, start=1):
        ...
        if not item.word:
            issues.append(ValidationIssue(location=location, message="Missing word."))
        if not item.definition:
            issues.append(ValidationIssue(location=location, message="Missing definition."))
        key = item.word.strip().lower()
        if key and key in seen_words:
            issues.append(
                ValidationIssue(location=location, message=f"Duplicate word: '{item.word}'.")
            )
        seen_words.add(key)
    return issues
...
def validate_exercise(questions: list[ExerciseQuestionData]) -> list[ValidationIssue]:
    if not questions:
        return [ValidationIssue(location="exercise", message="No questions found.")]

    issues = []
    for index, question in enumerate(questions, start=1):
        location = f"Question {index}"
        if not question.question_text:
            issues.append(ValidationIssue(location=location, message="Missing question text."))
        ...
        if not question.correct_answer:
            issues.append(ValidationIssue(location=location, message="Missing correct answer."))
